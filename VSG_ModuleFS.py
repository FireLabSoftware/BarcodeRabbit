...
## Version FR adds two routines for dealing with NGS file lists
##     vMetaFileLists collects a user-defined file list of NGS data from a simple list, wildcard specification, or file
##        A major feature of vMetaFileLists is that it automatically pairs R1/R2 files so that the output is a series of
##        read-file pairs (where pairing is possible) or single files.
##     vFastAToIter: works like vFastAToDict but returns a record-by-record iterator so memory is conserved.  Output of the iterator
##        is an iterative series of 2-ples (sequence id, sequence).
...
def vLog(*note,LogFileName=vDefaultLogFileName):
    ...
    if myThread1!=-1:
        note = 'Thread '+str(myThread1)+' '+note
    if myThread1==-1 and Threads1>1:
        note = 'Main_Program '+note
...
def vFileInfo(FileID):
    if type(FileID)==str:
        ...
        Fn1=FileID.name
    s11=os.stat(Fn1)
    return ','.join(["'"+os.path.basename(Fn1)+"'",
                     '#',
...
def vLogOpeningFile(FileID,kw):
    ...
    for k in kw:
        kwlist+=k+'='+str(kw[k])+' '
    vLog("opening file "+vFileInfo(FileID)+'  '+kwlist)
def vLogClosingFile(FileID):
    vLog("closing file "+vFileInfo(FileID))
def vLogRunningFile(FileID):
    vLog("running file"+vFileInfo(FileID))
...
if vv_argv:
    ...
    for vv_a1 in vv_argv:
        ...
        if not(vSpawned) and (myThread1==-1): print("Setting "+vv_VarName+'='+str(vv_NewVarValue))
...
def vFastAToDict(Fn, upper=False, lower=False, stripN=False, idwords='',OneReference=False):
    ...
    return D
class vFastAToIter():
    '''input is a fastA file, output is an iterator that yields (name,sequence) combinations.
        lower=True or upper=True results in a lower or upper cases output'''
    def __init__(self,F, upper=False, lower=False,stripN=False):
        self.upper = upper
        self.lower = lower
        self.stripN = stripN
        self.F = chain(F,['>x',])
        ...
        for L in self.F:
            if L[0]=='>':
                if S:
                    myS = ''.join(S)
                    if self.stripN:
                        myS = myS.replace('N','').replace('n','')
                    if self.upper:
                        myS = myS.upper()
                    elif self.lower:
                        myS = myS.lower()
                    myN = self.nextn
                    self.nextn = L.strip()[1:]
                    return(myN,myS)
                else:
                    self.nextn = L.strip()[1:]                
            else:
    ...
    if not(type(n)==str):
        ...
        for cc1,cc2 in zip_longest(n[0],n[1]):
            if cc1==cc2:
                ...
                newN += 'x'
        n = newN
    return os.path.basename(n).split('.gz')[0].split('.fa')[0]
def vmultiglob(x):
    if not(type(x))==str:
        x = ','.join(x)
    l = []
    ...
    return l
class vSampleDataClass():
    def __init__(self,files,sampleName): ## optional keys: sampleColor=, sampleCategory=
        if type(files)==str:
            files = (files,)
        self.files = files  ## single file name for unpaired, tuple for paired
        self.sampleName = sampleName
def vFileListMnemonic(FL):
    mn  = ''
...
def vMetaFileLists(DataFiles):
    ## Specify one or many NGS datasets for analysis.
    ## Input ('DataFiles') can be an implicit of explicit specificastion of a set of NGS files (fasta or fastq, gzipped or not)
    ## The function returns an explicit list of paired (where possible) or unpaired (where no pairing is clear) file tuples
    ## Input can be an explicit file list, a wild-card shorthand, or a file with a list of filenames to process
    ## For a file as input, one sample per line is the standard, with any number of corresponding files listed (e.g. tab delimited)
    ## the first column in the file can be a sample name that will refer to the filenames in that line
    ## more than one line can be assigned the same filename.  To feed this function a file of file locations, either give that file
    ## a suffix .files or prepend the filename with @ (e.g. vMetaFileLists('@myfilelist.txt'))
    ##
    ## vMetaFileLists returns a list of objects to be fed to an analysis routine.  The objects all have the attributes
    ##   .files: a tuple of the files corresponding to that object (generally two objects if there are paired ends, more or less for special cases)
    ##   .sampleName: a sample name to be used in describing this (and any other related object)
    ##   .sampleColor: a color to associate with a sample (specified in the .files file with syntax (color='green')
    ##   .sampleCategory: a category to associate with a sample (specified in the .files file with syntax (category='trees')
    ##     Sample Color and Category specifications can be anywhere on a line with samplename and associated files (generally set off by tabs)
    DataFileD = {}
    SampleLabelColor = Counter()
    SampleCategory = Counter()
    if type(DataFiles)==str and (DataFiles.lower().endswith('.files') or DataFiles.startswith('@')):
        if DataFiles.startswith('@'): DataFiles = DataFiles[1:].strip()
        try:
            s2d = vOpen(DataFiles,mode='rt').readlines()
        except:
            vLog('Failed to open metadata file ',DataFiles,'check to see that this file exists')
            exit()
        for j,L1 in enumerate(s2d):
            mySample = L1.split('\t')[0].strip()
            if not(mySample):
                mySample = os.path.basename(DataFiles)+"_Line_"+str(j)
            for k,f1 in enumerate(L1.replace(',',' ').replace('; ',';').replace('\t',' ').strip().split()):
                if ';' in f1:
                    f1,f2 = f1.split(';',1)
                    f1 = f1.strip(' "\t,'+"'").strip()
                    f2 = f2.strip(' "\t,'+"'").strip()
                    if (f1.lower().endswith('.fasta.gz') or f1.lower().endswith('.fa.gz') or f1.lower().endswith('.faa.gz') or f1.lower().endswith('.fastq.gz') or \
                        f1.lower().endswith('.fasta') or f1.lower().endswith('.fa') or f1.lower().endswith('.faa') or f1.lower().endswith('.fastq')) and os.path.isfile(f1) and \
                        (f2.lower().endswith('.fasta.gz') or f2.lower().endswith('.fa.gz') or f2.lower().endswith('.faa.gz') or f2.lower().endswith('.fastq.gz') or \
                        f2.lower().endswith('.fasta') or f2.lower().endswith('.fa') or f2.lower().endswith('.faa') or f2.lower().endswith('.fastq')) and os.path.isfile(f2):
                        if k==0: mySample = os.path.basename(DataFiles)+"_Line_"+str(j)
                        DataFileD[(f1,f2)] = mySample
                else:
                    f1 = f1.strip(' "\t,'+"'").strip()
                    for ff1 in sorted(list(glob(f1))):
                        if (ff1.lower().endswith('.fasta.gz') or ff1.lower().endswith('.fa.gz') or ff1.lower().endswith('.faa.gz') or ff1.lower().endswith('.fastq.gz') or \
                            ff1.lower().endswith('.fasta') or ff1.lower().endswith('.fa') or ff1.lower().endswith('.faa') or ff1.lower().endswith('.fastq')) and os.path.isfile(ff1):
                            if k==0: mySample = os.path.basename(DataFiles)+"_Line_"+str(j)
                            DataFileD[ff1] = mySample
            for f1 in L1.split('\t'):
                f1 = f1.strip()
                if f1.lower().startswith('rgb(') and f1.endswith(')'):
                    SampleLabelColor[mySample] = f1.lower()
                elif f1.lower().startswith('color(') and f1.endswith(')'):
                    SampleLabelColor[mySample] = f1.lower().split('(')[-1].split(')')[0]
                elif f1.lower().startswith('category(') and f1.endswith(')'):
                    SampleCategory[mySample] = f1.split('(')[-1].split(')')[0]    
    else:
        DataFileList = []
        for s11 in vmultiglob(DataFiles):
            if not(s11): continue
            s11 = s11.strip().strip('"').strip("'")
            if os.path.isdir(s11):
                DataFileList.extend(vmultiglob(os.path.join(s11,'*.fasta'))+
                                     vmultiglob(os.path.join(s11,'*.fastq'))+
                                     vmultiglob(os.path.join(s11,'*.faa'))+
                                     vmultiglob(os.path.join(s11,'*.faa.gz'))+
                                     vmultiglob(os.path.join(s11,'*.fa'))+
                                     vmultiglob(os.path.join(s11,'*.fa.gz'))+
                                     vmultiglob(os.path.join(s11,'*.fasta.gz'))+
                                     vmultiglob(os.path.join(s11,'*.fastq.gz')))
            elif os.path.isfile(s11):
                DataFileList.append(s11)
            elif list(glob(s11)):
                DataFileList.extend(sorted(list(glob(s11))))
            elif ';' in s11:
                (sf1,sf2) = s11.split(';',1)
                sf1 = sf1.strip("' "+'"')
                sf2 = sf2.strip("' "+'"')
                if os.path.isfile(sf1) and os.path.isfile(sf2):
                    DataFileList.append((sf1,sf2))
                else:
                    DataFileList.extend(sorted(list(glob(sf1)+list(glob(sf2)))))
        DataFileList.sort(key=str)
        i1 = 0
        while i1<len(DataFileList):
            if i1==len(DataFileList)-1:
                DataFileD[DataFileList[i1]] = vTruncName(DataFileList[i1])
                break
            myPair = (DataFileList[i1],DataFileList[i1+1])
            myPairName = vAreWeAPair(*myPair)
            if myPairName:
                DataFileD[myPair] = vTruncName(myPairName)
                i1 += 2
            else:
                DataFileD[DataFileList[i1]] = vTruncName(DataFileList[i1])
                i1 += 1
    SampleDataList = []
    for d in DataFileD:
        mySample = DataFileD[d]
        SampleDataList.append(vSampleDataClass(d,DataFileD[d]))
        if mySample in SampleLabelColor:
            SampleDataList[-1].sampleColor = SampleLabelColor[mySample]
        else:
            SampleDataList[-1].sampleColor = 0
        if mySample in SampleCategory:
            SampleDataList[-1].sampleCategory = SampleCategory[mySample]
        else:
            SampleDataList[-1].sampleCategory = 'AllCategory'
...
if not(vSpawned) and (myThread1==-1): print('Successful import of VSG Version='+VSGVersion)
